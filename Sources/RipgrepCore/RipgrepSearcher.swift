...
public struct RipgrepSearcher: @unchecked Sendable {
    ...
    func writeDarwinSimpleByteLiteralLines(
        ...
        if let fixedResetStartFastPath {
            return writeDarwinFixedResetStartFastPath(
                data,
                fileURL: fileURL,
        ...
        if let byteUnitFastPath {
            ...
            return writeDarwinByteUnitFastPath(
                data,
                fileURL: fileURL,
                pattern: byteUnitFastPath.pattern,
                unicodeStartOnly: byteUnitFastPath.unicodeStartOnly,
        ...
        data.withUnsafeBytes { rawBytes in
            ...
            if (canDirectWriteLiteralReplacement || canDirectWriteOnlyMatchingLiteralReplacement),
               ...
               let literal = literals.first {
                ...
                while searchOffset < data.count, matchedLineCount < maxCount {
                    ...
                    if !onlyMatching,
                       pathPrefixBytes != nil || wantsLineNumber || options.column || options.byteOffset {
                        advanceLineNumber(to: lineStart)
                        writePathPrefixIfNeeded()
            ...
            if canDirectWriteVimgrep || canDirectWriteVimgrepLiteralReplacement {
                ...
                func vimgrepLineBounds(containing matchStart: Int) -> (lineStart: Int, lineEnd: Int) {
                    var lineStart = matchStart
                    while lineStart > 0, baseAddress[lineStart - 1] != UInt8(ascii: "\n") {
                        lineStart -= 1
                    }
                ...
                func writeVimgrepPrefixes(lineNumber: Int, column: Int, byteOffset: Int) {
                    if let pathBytes {
                        ...
                        withUnsafeBytes(of: &separator) { buffer in
                            writeBytes(buffer)
                        }
                    }
            ...
            if literals.count > 1,
               ...
               !canDirectWriteTransformedMultiLiteralLines {
                ...
                func parallelLineAlignedChunkRanges(
                ...
                func collectChunkedParallelMultiLiteralLineBounds() -> [DarwinMatchedLineBound]? {
                    guard let chunkRanges = parallelLineAlignedChunkRanges(
                ...
                func collectChunkedParallelSuffixLiteralLineBounds(
                    _ plans: [SuffixLiteralPlan]
                ) -> [DarwinMatchedLineBound]? {
                    guard let chunkRanges = parallelLineAlignedChunkRanges(
                        minimumChunkCount: plans.count
                    ) else {
                        return nil
                    }
                    ...
                    DispatchQueue.concurrentPerform(iterations: chunkRanges.count) { chunkIndex in
                        ...
                        func literalMatches(_ literal: [UInt8], at offset: Int) -> Bool {
                            guard offset >= range.start, literal.count <= range.end - offset else {
                                return false
                            }
                        ...
                        for plan in plans where plan.suffix.count <= range.end - range.start {
                ...
                if let suffixPlans = uniqueLastWordSuffixPlans() {
                    if shouldCollectParallelMultiLiteralLineBounds() {
                        matchedLineBounds = collectChunkedParallelSuffixLiteralLineBounds(suffixPlans)
                            ?? collectParallelSuffixLiteralLineBounds(suffixPlans)
                    } else {
                    ...
                    matchedLineBounds = collectChunkedParallelMultiLiteralLineBounds()
                        ?? collectParallelMultiLiteralLineBounds()
                } else {
                    for literal in literals where literal.count <= data.count {
                        var searchOffset = 0
                        literal.withUnsafeBufferPointer { needle in
            ...
            prefixBytesStorage.withUnsafeBufferPointer { prefixBytes in
                literalBytesStorage.withUnsafeBufferPointer { literalBytes in
                    suffixBytesStorage.withUnsafeBufferPointer { suffixBytes in
                        ...
                        let prefixBaseAddress = prefix == nil ? nil : prefixBytes.baseAddress
    ...
    private func writeDarwinFixedResetStartFastPath(
        _ data: Data,
        fileURL: URL,
        prefix: [UInt8],
        literal: [UInt8],
        ...
        let stopAfterFirstMatch = options.quiet || filesWithMatches || filesWithoutMatch
        let maxCount = options.maxCount ?? Int.max
        let needleStorage = caseInsensitiveASCII ? needle.map(asciiLowercase) : needle
        var caseInsensitiveShifts = [Int](repeating: needle.count, count: 256)
        if caseInsensitiveASCII && needleStorage.count > 1 {
            for index in 0..<(needleStorage.count - 1) {
                caseInsensitiveShifts[Int(needleStorage[index])] = needleStorage.count - 1 - index
            }
        }
        ...
        data.withUnsafeBytes { rawBytes in
            ...
            let baseAddress = rawBaseAddress.assumingMemoryBound(to: UInt8.self)
            needleStorage.withUnsafeBufferPointer { needleBytes in
                guard let needleBase = needleBytes.baseAddress,
                      needleBytes.count <= data.count else {
                    return
                }
    ...
    private func writeDarwinByteUnitFastPath(
        _ data: Data,
        fileURL: URL,
        ...
        data.withUnsafeBytes { rawBytes in
            ...
            func recordMatch(start: Int, end: Int) {
                totalMatchCount += 1
                if stopAfterFirstMatch || (countOnly && matchedLineCount + 1 == maxCount) {
                    bytesSearched = end
                }
                if onlyMatching {
                    writeDarwinOnlyMatchingPrefixes(
                    ...
                    writeBytes(UnsafeRawBufferPointer(
                        start: rawBaseAddress.advanced(by: start),
                        count: end - start
                    ))
                    var newline = UInt8(ascii: "\n")
                    withUnsafeBytes(of: &newline) { buffer in
            ...
            while lineStart < data.count, !shouldStop {
                ...
                let matchesBeforeLine = totalMatchCount

                switch pattern {
                case .single:
                    var offset = lineStart
                    while offset < lineEnd, !shouldStop {
                        if isAllowedStart(offset) {
                            recordMatch(start: offset, end: offset + 1)
                        }
                        offset += 1
                        if stopAfterFirstMatch || (countOnly && matchedLineCount + 1 == maxCount) {
                            shouldStop = totalMatchCount > matchesBeforeLine
                        }
                    }
                case .oneOrMore:
                    var offset = lineStart
                    ...
                    if offset < lineEnd {
                        recordMatch(start: offset, end: lineEnd)
                    }
                case .fixed(let count):
                    var offset = lineStart
                    while offset < lineEnd, lineEnd - offset >= count, !shouldStop {
                        if isAllowedStart(offset) {
                            recordMatch(start: offset, end: offset + count)
                            offset += count
                        } else {
                            offset += 1
                        }
                ...
                if totalMatchCount > matchesBeforeLine {
                    matchedLineCount += 1
                    if stopAfterFirstMatch || (countOnly && matchedLineCount == maxCount) {
                        shouldStop = true
                    }
    ...
    private func isDarwinUTF8ContinuationByte(_ byte: UInt8) -> Bool {
        byte & 0xC0 == 0x80
    }
    ...
    private func searchFile(
        ...
        func streamedSearchOutcome() throws -> FileSearchOutcome? {
            ...
            if matcher.asciiFixedClassSequenceFastPath() != nil,
               canUseBufferedRawLiteralSearch(fileURL: fileURL) {
                return nil
            }
    ...
    private func searchRawASCIIFixedClassSequenceContents(
        ...
        if lineOutput {
            ...
            let includeSpans = options.json
                ...
                || options.column
                || !canOmitMatchSpans(options: options)
            let scan = data.withUnsafeBytes { rawBuffer -> ASCIIFixedClassLineScan? in
                let bytes = rawBuffer.bindMemory(to: UInt8.self)
                guard let baseAddress = bytes.baseAddress else {
            ...
            return SearchFileResult(
                ...
                bytesSearched: scan.bytesSearched,
                searched: true
            )
        }
    ...
    private func searchRawASCIIRunSuffixContents(
        ...
        let dataCount = data.count
        let contentStart = data.starts(with: [0xEF, 0xBB, 0xBF]) ? 3 : 0
        let searchableCount = dataCount - contentStart
        guard searchableCount > suffix.count else {
            return SearchFileResult(
                fileURL: fileURL,
                matches: [],
    ...
    private func asciiFixedClassLineMatches(
        ...
        if !reachedMaxCount,
           lineStart < dataCount || dataCount == 0 || baseAddress[dataCount - 1] != newline {
    ...
    private func searchRawRegexPrefilterContents(
        ...
        if let fastResult = searchDarwinWordPrefixLiteralContents(
            data,
            fileURL: fileURL,
            matcher: matcher,
            options: options
    ...
    private func searchDarwinWordPrefixLiteralContents(
        _ data: Data,
        fileURL: URL,
        matcher: PatternMatcher,
        options: RipgrepOptions
        ...
        guard options.printMode == .matchingLines,
              options.heading != true,
              canOmitMatchSpans(options: options),
              let pattern = wordPrefixLiteralPattern(options: options) else {
            return nil
        }
        ...
        let result = data.withUnsafeBytes { rawBytes -> SearchFileResult? in
            ...
            func nextLiteralStart(from offset: Int, through limit: Int) -> Int? {
                guard offset < limit, pattern.literal.count <= limit - offset else {
                    return nil
                }
                let foundPointer = pattern.literal.withUnsafeBufferPointer { needle in
                    rg_memmem_simple(
                        baseAddress.advanced(by: offset),
                        limit - offset,
                        needle.baseAddress,
            ...
            func lineMatches(lineStart: Int, lineEnd: Int, firstLiteralStart: Int) -> Bool? {
                var literalStart: Int? = firstLiteralStart
                while let currentLiteralStart = literalStart {
                    guard currentLiteralStart > lineStart else {
                        literalStart = nextLiteralStart(from: currentLiteralStart + 1, through: lineEnd)
                        continue
                    }
                    let previousByte = bytes[currentLiteralStart - 1]
                    if previousByte < 0x80 {
                        if isASCIIRegexWordByte(previousByte) {
                            return true
                        }
                    } else if !pattern.asciiOnly {
                        return nil
                    }
                    literalStart = nextLiteralStart(from: currentLiteralStart + 1, through: lineEnd)
                }
            ...
            while searchOffset < dataCount {
                ...
                if let asciiMatch = lineMatches(
                    ...
                    matched = !decodedLiteralFallbackSpans(
                        matcher: matcher,
                        options: options,
                        line: line
                    ).isEmpty
    ...
    private func wordPrefixLiteralPattern(options: RipgrepOptions) -> (literal: [UInt8], asciiOnly: Bool)? {
        ...
        var asciiOnly = options.noUnicode
        ...
        guard let literal = RegexLiteralParser.literal(fromPlainRegexPattern: String(pattern[literalStart...])),
    ...
    private func asciiRunSuffixPattern(options: RipgrepOptions) -> ASCIIRunSuffixPattern? {
        ...
        if pattern.hasPrefix("[A-Z]+") {
            ...
            pattern.removeFirst("[A-Z]{".count)
            guard let parsed = parsePositiveDecimalPrefix(from: &pattern),
                  pattern.hasPrefix("}") else {
                return nil
            }
            pattern.removeFirst()
            run = .exactUppercase(parsed)
    ...
    private func asciiRunMatches(
        ...
        for index in start..<end where !predicate(bytes[index]) {
            return false
        }
        return true
    }
    ...
    private func parsePositiveDecimalPrefix(from pattern: inout String) -> Int? {
        ...
        while let first = pattern.utf8.first,
              first >= UInt8(ascii: "0"),
              first <= UInt8(ascii: "9") {
            value = value * 10 + Int(first - UInt8(ascii: "0"))
