...
public enum SwiftDarwinLiteralPreflight {
    ...
    private static func runSortedDirectory(
        ...
        let relativePathStart = rootPath.utf8.count + 1
        files.sort {
            sortedPathBytesPrecedeByComponents(
                $0.pathBytes,
                $1.pathBytes,
                startingAt: relativePathStart
            )
        }
    ...
    private static func sortedPathBytesPrecedeByComponents(
        _ lhs: [UInt8],
        _ rhs: [UInt8],
        startingAt relativePathStart: Int
    ) -> Bool {
        var leftStart = relativePathStart
        var rightStart = relativePathStart
        while true {
