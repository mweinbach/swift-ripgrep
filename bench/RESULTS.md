...
thread the already-computed root Git/VCS context through explicit, global, and
parent ignore setup. This removes the remaining repeated repository-state probes
from the root ignore-loading path without changing traversal order or ignore
semantics.

Status/stdout/stderr matched the saved `83470a1` Swift binary and Rust for
default, hidden, no-vcs, and explicit-ignore file listing. Search-mode
status/stdout/stderr also matched both oracles for quiet hit, quiet miss, and an
explicit-ignore quiet hit. `swift build -c release` passed before benchmarking.

| Case | Patched Swift | Baseline `83470a1` | Rust reference |
| --- | ---: | ---: | ---: |
| `--files linux` | 103.3 ms mean / 90.8-125.9 ms range | 106.2 ms / 94.0-132.2 ms | 102.4 ms / 82.1-202.6 ms |
| `--hidden --files linux` guard | 106.0 ms / 95.3-125.0 ms | 107.0 ms / 93.2-126.7 ms | 105.5 ms / 81.0-236.7 ms |
| `--no-ignore-vcs --ignore-file linux/.gitignore --files linux` | 96.9 ms / 85.5-116.7 ms | 101.5 ms / 83.2-125.7 ms | 103.9 ms / 76.2-212.1 ms |
| `-q SCHED linux` guard | 10.2 ms / 8.8-12.2 ms | 10.2 ms / 8.9-17.7 ms | 6.2 ms / 5.3-8.9 ms |
| `-q ABSENT_LITERAL_NOPE linux` guard | 1.294 s / 1.174-1.633 s | 1.448 s / 1.225-1.738 s | 3.689 s / 3.340-4.127 s |

Raw hyperfine exports:
`/tmp/swift-rg-bench/file-root-vcs-context-ab-1780522090.json` and
`/tmp/swift-rg-bench/search-root-vcs-context-ab-1780522142.json`.
...
`--no-ignore` string walker. Output stayed byte-for-byte identical to the
previous Swift binary and Rust for sorted visible no-ignore, reverse sorted
visible no-ignore, hidden no-ignore, default sorted files, and sorted
`--no-ignore-vcs` controls. The timing did not hold: the target forward row
slowed slightly, reverse sorted no-ignore slowed sharply, and the guard rows
all moved the wrong way. The source change was reverted.

| Case | Probe Swift | Baseline `7e63f65` | Rust reference |
| --- | ---: | ---: | ---: |
| `--sort path --no-ignore --files linux` | 177.42 ms median / 184.71 ms mean | 172.80 ms / 183.17 ms | 170.13 ms / 181.87 ms |
| `--sortr path --no-ignore --files linux` | 199.43 ms / 206.01 ms | 172.59 ms / 186.71 ms | 215.49 ms / 224.31 ms |
| `--sort path --hidden --no-ignore --files linux` guard | 178.57 ms / 192.85 ms | 176.04 ms / 184.77 ms | not remeasured |
| `--sort path --files linux` guard | 150.16 ms / 161.87 ms | 146.64 ms / 152.91 ms | not remeasured |
| `--sort path --no-ignore-vcs --files linux` guard | 129.17 ms / 129.43 ms | 126.66 ms / 126.97 ms | not remeasured |

Raw hyperfine export:
`/tmp/swift-rg-bench/noignore-marker-skip-ab-1780511600.json`.
...
At this checkpoint, bounded Swift Darwin word-regexp count-line output used the
same rejected ASCII word-candidate skip as bounded line output. Single-literal
and multi-literal count scanners still retained the 128 rejected-candidate
fallback for unbounded counts, but positive max-count searches advanced to the
end of the current ASCII regex word and kept scanning until the requested count
limit or end of file. A same-session A/B against the saved prepatch binary,
with Rust included, measured:
...
## Retained unbounded word-boundary count candidate skipping - 2026-06-03

The count-only ASCII word scanners now also use the rejected-candidate skip for
unbounded `-c` and `--count-matches` searches. Unlike line-output scanners,
count scanners do not buffer matching lines, so continuing through dense ASCII
boundary rejects avoids falling back to the broader searcher while preserving
the existing Unicode-boundary fallback. A same-session A/B against the saved
prepatch binary, with Rust included, measured:

| Case | Current Swift | Prepatch Swift | Rust |
| --- | ---: | ---: | ---: |
| `-w -c literal match-ascii-46m.txt` | 13.06 ms median / 13.08 ms mean | 112.20 ms / 110.99 ms | 41.50 ms / 41.48 ms |
| `-w --count-matches literal match-ascii-46m.txt` | 13.02 ms / 13.08 ms | 109.26 ms / 109.59 ms | 41.14 ms / 41.33 ms |
| `-w -c -m 1 literal match-ascii-46m.txt` control | 12.70 ms / 12.73 ms | 13.04 ms / 13.06 ms | 41.44 ms / 41.62 ms |

Release stdout, stderr, and status matched the sibling Rust oracle on dense
ASCII count, count-matches, absent count, absent count-matches, and bounded
count controls. Executable regression coverage now includes repeated
boundary-rejected unbounded word count and count-matches rows.

Raw hyperfine export:
`/tmp/swift-rg-bench/word-count-unbounded-skip-ab-1780476647.json`.
...
- Patched Swift matched Rust `rg` sorted file output for default `--files`,
...
sequences now reuse the Swift-only vimgrep line writer and return the total
match count, unique matched-line count, searched bytes, and printed bytes
needed for the stats summary. The route preserves vimgrep line-number, column,
byte-offset, and filename field toggles, while falling back for only-matching
vimgrep, quiet mode, JSON, color, trim, CRLF conversion, context, replacement,
passthru, semantic modes, UTF BOM inputs, and files with NUL bytes. This
remains Swift-only and does not add a C shim.

Validation:

- Patched Swift matched Rust after normalizing elapsed stats lines for
  `--stats --vimgrep` default, `--no-line-number`, `--no-column`, `-b`,
  multiple matches on one line, final no-newline output, no-match, UTF-8 BOM
  fallback, and early NUL fallback controls.
- Added regression coverage for fixed-class `--stats --vimgrep` matched
  output/status, including nonzero `bytes printed` accounting.
- `swift build -c release`,
  `swift test --filter MiscTests/darwinExecutableLiteralPreflightDenseLines`,
  full `swift test`,
  `SWIFT_RIPGREP_PARITY=1 swift test --filter ParityHarnessTests`,
...
Same-session probe at `868e645` measured `--stats --vimgrep` before this slice
at 70.9 ms mean for the late-hit fixture versus Rust at 23.2 ms, and 39.2 ms
for the no-hit fixture versus Rust at 22.9 ms. The patched run used 5 warm-ups
and 30 timed runs with stdout redirected to `/dev/null`. After the stats
vimgrep preflight:

| Case | Current Swift | Pre-change `868e645` | Rust |
| --- | ---: | ---: | ---: |
| `--stats --vimgrep '[A-Z]{5}' fixed-late-uppercase-46m.txt` | 23.3 ms mean / 22.3-24.4 ms range | 70.9 ms / 50.7-94.1 ms | 23.6 ms / 22.4-24.5 ms |
| `--stats --vimgrep '[A-Z]{5}' no-uppercase-46m.txt` | 14.2 ms / 13.9-14.7 ms | 39.2 ms / 23.7-57.1 ms | 24.4 ms / 23.2-27.2 ms |

Raw hyperfine exports:
`/tmp/swift-rg-bench/fixed-formatted-visible-sweep-868e645.json` and
`/tmp/swift-rg-bench/fixed-stats-vimgrep-patched-868e645.json`.
...
now reuse the Swift-only fixed-class only-matching writer for regular text
inputs. The route emits non-overlapping match spans with vimgrep line, column,
byte-offset, and filename fields, while preserving the existing fallback for
color, trim, CRLF conversion, JSON, stats, context, replacement, passthru, and
semantic modes. UTF BOM inputs and files with NUL bytes also fall back. This
remains Swift-only and does not add a C shim.

Validation:

- Patched Swift matched Rust for default `--vimgrep -o`, `--no-line-number`,
  `--no-column`, `--vimgrep -o -b`, multiple matches on one line, final
  no-newline output, no-match, UTF-8 BOM fallback, and early NUL fallback
  controls.
- Added regression coverage for fixed-class default vimgrep only-matching
  matched output/status.
- `swift build -c release` and
  `swift test --filter MiscTests/darwinExecutableLiteralPreflightDenseLines`
  passed before recording these results.

Same-session probe at `8b57836` measured `--vimgrep -o` before this slice at
67.4 ms mean for the late-hit fixture versus Rust at 20.9 ms. The patched run
used 5 warm-ups and 30 timed runs with stdout redirected to `/dev/null`. After
the vimgrep only-matching preflight:

| Case | Current Swift | Pre-change `8b57836` | Rust |
| --- | ---: | ---: | ---: |
| `--vimgrep -o '[A-Z]{5}' fixed-late-uppercase-46m.txt` | 20.0 ms mean / 19.2-25.4 ms range | 67.4 ms / 53.6-82.4 ms | 21.0 ms / 19.9-25.4 ms |
| `--vimgrep -o -b '[A-Z]{5}' fixed-late-uppercase-46m.txt` | 19.4 ms / 19.0-20.1 ms | not remeasured | 20.4 ms / 20.0-20.9 ms |
| `--vimgrep -o '[A-Z]{5}' no-uppercase-46m.txt` | 11.4 ms / 10.7-14.4 ms | 42.5 ms / 36.5-54.4 ms | 20.4 ms / 20.0-20.7 ms |

Raw hyperfine exports:
`/tmp/swift-rg-bench/fixed-vimgrep-only-matching-probe-8b57836.json` and
`/tmp/swift-rg-bench/fixed-vimgrep-only-matching-patched-8b57836.json`.
...
regex sequences now use the Swift matched-stats scanner for regular text
inputs. The preflight writes only the count output through the existing count
writer and preserves the existing no-match count proof, while UTF BOM inputs
and files with NUL bytes fall back to the existing machinery. This remains
Swift-only and does not add a C shim.

Validation:

- Patched Swift matched Rust for count hit, count-matches hit, explicit
  `--with-filename` count prefix, include-zero hit, CRLF count hit, count
  no-match, count-matches sparse no-match, UTF-8 BOM fallback, UTF-16LE BOM
  fallback, and early/late NUL fallback controls.
- Added regression coverage for fixed-class count, count-matches, and
  explicitly prefixed count matched output/status.
- `swift build -c release` and
  `swift test --filter MiscTests/darwinExecutableLiteralPreflightDenseLines`
  passed before recording these results.

Same-session pre-probe at `d44dbe2` measured the matched plain count gap
before this slice. The pre-change sweep used 5 warm-ups and 25 timed runs; the
patched run used 5 warm-ups and 30 timed runs. After the matched count
preflight:

| Case | Current Swift | Pre-change `d44dbe2` | Rust |
| --- | ---: | ---: | ---: |
| `-c '[A-Z]{5}' fixed-late-uppercase-46m.txt` | 16.7 ms mean / 15.4-20.0 ms range | 40.2 ms / 25.1-58.2 ms | 20.9 ms / 19.4-28.0 ms |
| `--count-matches '[A-Z]{5}' fixed-late-uppercase-46m.txt` | 16.1 ms / 15.3-16.9 ms | 62.5 ms / 25.8-92.5 ms | 20.4 ms / 19.7-21.4 ms |
| `-c '[A-Z]{5}' no-uppercase-46m.txt` | 11.0 ms / 10.4-12.6 ms | not remeasured | 20.5 ms / 19.6-21.7 ms |
| `--count-matches '[A-Z]{5}' sparse-uppercase-no-five-46m.txt` | 15.1 ms / 14.2-16.6 ms | not remeasured | 21.0 ms / 20.0-21.9 ms |

Raw hyperfine exports:
`/tmp/swift-rg-bench/fixed-class-matched-mode-sweep-d44dbe2.json` and
`/tmp/swift-rg-bench/fixed-count-matched-patched-d44dbe2.json`.
...
ASCII fixed-class regex sequences now use the Swift matched-stats preflight for
regular text inputs. The preflight reuses the fixed-class summary scanner,
writes the count output, flushes, and appends the existing deterministic stats
summary. No-match count modes still hit the no-match proof first, while UTF BOM
inputs and files with NUL bytes fall back to the existing machinery. This
remains Swift-only and does not add a C shim.

Validation:

- Patched Swift matched Rust for stats count hit, count-matches hit, count
  no-match, count-matches sparse no-match, CRLF count hit, UTF-8 BOM fallback,
  UTF-16LE BOM fallback, and early/late NUL fallback controls after normalizing
  elapsed timing fields.
- Added regression coverage for fixed-class stats count and count-matches
  matched output/status.
- `swift build -c release` and
  `swift test --filter MiscTests/darwinExecutableLiteralPreflightDenseLines`
  passed before recording these results.

Same-session pre-probe at `747d25c` measured the matched count-stats gap before
this slice. These section-specific `hyperfine` runs used 5 warm-ups and 30
timed runs per case. After the matched count-stats preflight:

| Case | Current Swift | Pre-change `747d25c` | Rust |
| --- | ---: | ---: | ---: |
| `--stats -c '[A-Z]{5}' fixed-late-uppercase-46m.txt` | 13.0 ms mean / 11.3-19.7 ms range | 40.0 ms / 21.5-56.8 ms | 18.5 ms / 17.8-23.0 ms |
| `--stats --count-matches '[A-Z]{5}' fixed-late-uppercase-46m.txt` | 12.5 ms / 12.0-16.0 ms | 34.4 ms / 19.7-43.6 ms | 21.3 ms / 17.6-26.7 ms |
| `--stats -c '[A-Z]{5}' no-uppercase-46m.txt` | 9.7 ms / 7.7-13.9 ms | 7.2 ms / 7.0-7.4 ms | 19.6 ms / 17.6-30.1 ms |
| `--stats --count-matches '[A-Z]{5}' sparse-uppercase-no-five-46m.txt` | 11.3 ms / 10.9-13.3 ms | 10.8 ms / 10.5-11.5 ms | 18.4 ms / 17.7-18.8 ms |

Raw hyperfine exports:
`/tmp/swift-rg-bench/fixed-stats-count-matched-current-747d25c.json` and
`/tmp/swift-rg-bench/fixed-stats-count-matched-patched-747d25c.json`.
...
fixed-class regex sequences now use the Swift no-match byte-count proof before
falling through to the general stats path. Proven misses write the path, flush,
and append the existing deterministic no-match stats summary with exit 0.
...
- Patched Swift matched Rust for stats files-without-match text miss, text hit
  fallback, binary-prefix miss fallback, and UTF-8 BOM hit fallback after
  normalizing elapsed timing fields.
- Added regression coverage for fixed-class stats files-without-match no-match
  path plus stats summary output/status.
- `swift build -c release` and
  `swift test --filter MiscTests/darwinExecutableLiteralPreflightDenseLines`
  passed before recording these results.

A same-session quick pre-probe before this slice measured Swift at 38.7 ms mean
for `--stats --files-without-match '[A-Z]{5}' no-uppercase-46m.txt` versus Rust
at 17.9 ms. After the patch:

| Case | Current Swift | Rust |
| --- | ---: | ---: |
| `--stats --files-without-match '[A-Z]{5}' no-uppercase-46m.txt` | 7.9 ms mean / 7.1-15.5 ms range | 18.2 ms / 17.5-21.1 ms |
| `--stats --files-without-match '[A-Z]{5}' fixed-late-uppercase-46m.txt` | 33.9 ms / 18.7-43.3 ms | 17.8 ms / 17.4-18.5 ms |

Raw hyperfine export:
`/tmp/swift-rg-bench/fixed-stats-fileswithoutmatch-patched-9b67259.json`.
...
the staged SIMD16 helper shape as well. The helper first checks the generic
first/middle/tail bytes and then runs a full nine-byte SIMD proof only when
candidate lanes exist, avoiding the generic `memcmp` verifier for dense
near-misses such as `tqetazqet` over repeated `theta zet` windows.

Validation:

- Added a nine-byte `tqetazqet` regression test with repeated `theta zet`
  false candidates and a positive hit line.
- Current Swift stdout/stderr/status matched checkpoint
  `/tmp/swift-rg-bench/baseline-5e510dd-1780357822-ripgrep` and Rust for
  nine-byte, eight-byte, seven-byte, six-byte, three-byte, longer-literal, and
  quiet controls. The stats case matched Rust after normalizing elapsed-time
  lines.
- `swift build -c release` and the focused 2/3/4/5/6/7/8/9-byte scanner tests
  passed before benchmarking.

A same-session 30-run hyperfine A/B against checkpoint `5e510dd`, with five
warmups, measured:

| Command | Current Swift | Checkpoint `5e510dd` | Rust |
| --- | ---: | ---: | ---: |
| `-n tqetazqet no-match-ascii-46m.txt` | 11.9 ms mean / 10.5-15.0 ms range | 20.6 ms / 19.6-21.7 ms | 8.8 ms / 8.3-9.4 ms |
| `--stats --files-without-match tqetazqet no-match-ascii-46m.txt` | 10.9 ms / 9.3-12.1 ms | 19.4 ms / 18.2-20.5 ms | 8.0 ms / 7.3-9.2 ms |
| `-n zzzzzzzzz no-match-ascii-46m.txt` guardrail | 10.0 ms / 9.1-10.7 ms | 11.3 ms / 10.2-12.0 ms | 9.0 ms / 8.2-9.8 ms |
| `-n tqetazqe no-match-ascii-46m.txt` guardrail | 12.0 ms / 10.9-14.2 ms | 11.5 ms / 10.1-13.1 ms | not remeasured in this pass |
| `-n " qqeqq " no-match-ascii-46m.txt` guardrail | 12.4 ms / 11.3-13.3 ms | 12.1 ms / 11.2-12.9 ms | not remeasured in this pass |
| `-n "tqetq " no-match-ascii-46m.txt` guardrail | 11.6 ms / 10.2-13.8 ms | 11.4 ms / 10.3-12.2 ms | not remeasured in this pass |
| `-n exa no-match-ascii-46m.txt` guardrail | 10.2 ms / 9.3-11.1 ms | 9.8 ms / 8.6-10.5 ms | not remeasured in this pass |
| `-n missingliteral match-ascii-46m.txt` guardrail | 72.3 ms / 71.2-73.8 ms | 72.9 ms / 71.5-74.0 ms | not remeasured in this pass |
| `-q PM_RESUME linux` guardrail | 36.2 ms / 34.1-45.2 ms | 35.7 ms / 34.5-37.0 ms | not remeasured in this pass |

Raw hyperfine exports:
`/tmp/swift-rg-bench/ninebyte-specialized-probe-1780358102.json` and
`/tmp/swift-rg-bench/ninebyte-specialized-guard-1780358124.json`.

## Eight-byte Swift memmem staged SIMD helper — 2026-06-01

The no-C-shim Swift memmem fallback now routes eight-byte exact literals through
a staged SIMD16 helper. It keeps the cheap first/middle/tail mask for ordinary
no-candidate chunks, then runs a full eight-byte SIMD proof only for candidate
lanes. This covers dense verifier near-misses such as `tqetazqe` over repeated
`theta ze` windows without charging unrelated eight-byte misses for eight
unaligned vector loads.

Validation:

- Added an eight-byte `tqetazqe` regression test with repeated `theta ze` false
  candidates and a positive hit line.
- Current Swift stdout/stderr/status matched checkpoint
  `/tmp/swift-rg-bench/baseline-bbaafe2-1780357091-ripgrep` and Rust for
  eight-byte, seven-byte, six-byte, five-byte, three-byte, longer-literal, and
  quiet controls. The stats case matched Rust after normalizing elapsed-time
  lines.
- `swift build -c release` and the focused 2/3/4/5/6/7/8-byte scanner tests
  passed before benchmarking.

A same-session 30-run hyperfine A/B against checkpoint `bbaafe2`, with five
warmups, measured:

| Command | Current Swift | Checkpoint `bbaafe2` | Rust |
| --- | ---: | ---: | ---: |
| `-n tqetazqe no-match-ascii-46m.txt` | 11.5 ms mean / 10.4-13.6 ms range | 20.3 ms / 18.7-21.6 ms | 8.6 ms / 8.0-10.1 ms |
| `--stats --files-without-match tqetazqe no-match-ascii-46m.txt` | 10.1 ms / 9.0-11.2 ms | 18.9 ms / 17.8-20.4 ms | 7.6 ms / 7.0-8.7 ms |
| `-n zzzzzzzz no-match-ascii-46m.txt` guardrail | 9.7 ms / 8.6-10.7 ms | 11.1 ms / 10.0-12.1 ms | 8.9 ms / 8.1-9.6 ms |
| `-n " qqeqq " no-match-ascii-46m.txt` guardrail | 12.6 ms / 12.0-14.1 ms | 12.4 ms / 11.8-13.1 ms | not remeasured in this pass |
| `-n "tqetq " no-match-ascii-46m.txt` guardrail | 11.4 ms / 10.4-13.0 ms | 11.1 ms / 9.8-12.3 ms | not remeasured in this pass |
| `-n tqexa no-match-ascii-46m.txt` guardrail | 10.6 ms / 9.6-11.6 ms | 10.4 ms / 9.6-11.2 ms | not remeasured in this pass |
| `-n exa no-match-ascii-46m.txt` guardrail | 9.5 ms / 8.6-10.2 ms | 9.7 ms / 8.7-10.4 ms | not remeasured in this pass |
| `-n missingliteral match-ascii-46m.txt` guardrail | 71.8 ms / 70.5-73.1 ms | 72.2 ms / 70.0-73.9 ms | not remeasured in this pass |
| `-q PM_RESUME linux` guardrail | 35.1 ms / 33.7-36.1 ms | 35.1 ms / 33.9-36.8 ms | not remeasured in this pass |

Raw hyperfine exports:
`/tmp/swift-rg-bench/eightbyte-specialized-probe-1780357383.json` and
`/tmp/swift-rg-bench/eightbyte-specialized-guard-1780357405.json`.

## Seven-byte Swift memmem fixed-shape SIMD helper — 2026-06-01

The no-C-shim Swift memmem fallback now routes seven-byte exact literals
through a fixed-shape SIMD16 helper. This covers dense leading/trailing-space
near-misses such as ` qqeqq ` over repeated ` theta ` windows, where the
generic scanner accepts on first/middle/final bytes and then rejects inside the
verifier.

Validation:

- Added a seven-byte ` qqeqq ` regression test with repeated `theta` false
  candidates and a positive hit line.
- Current Swift stdout/stderr/status matched checkpoint
  `/tmp/swift-rg-bench/baseline-e3f652b-1780356446-ripgrep` and Rust for
  seven-byte, six-byte, five-byte, four-byte, three-byte, longer-literal, and
  quiet controls. The stats case matched Rust after normalizing elapsed-time
  lines.
- `swift build -c release` and the focused 2/3/4/5/6/7-byte scanner tests
  passed before benchmarking.

A same-session 30-run hyperfine A/B against checkpoint `e3f652b`, with five
warmups, measured:

| Command | Current Swift | Checkpoint `e3f652b` | Rust |
| --- | ---: | ---: | ---: |
| `-n " qqeqq " no-match-ascii-46m.txt` | 11.9 ms mean / 11.4-12.8 ms range | 20.9 ms / 20.1-21.8 ms | 8.8 ms / 8.2-9.4 ms |
| `--stats --files-without-match " qqeqq " no-match-ascii-46m.txt` | 10.2 ms / 9.6-11.0 ms | 18.8 ms / 17.8-20.4 ms | 8.1 ms / 7.4-9.0 ms |
| `-n zzzzzzz no-match-ascii-46m.txt` guardrail | 12.0 ms / 11.1-12.7 ms | 11.6 ms / 10.9-12.0 ms | 9.1 ms / 8.5-10.0 ms |
| `-n "tqetq " no-match-ascii-46m.txt` guardrail | 10.8 ms / 9.9-12.0 ms | 10.9 ms / 9.8-12.0 ms | not remeasured in this pass |
| `-n tqexa no-match-ascii-46m.txt` guardrail | 9.9 ms / 9.2-10.9 ms | 10.0 ms / 9.3-10.6 ms | not remeasured in this pass |
| `-n btta no-match-ascii-46m.txt` guardrail | 9.8 ms / 9.0-11.9 ms | 10.0 ms / 8.8-11.5 ms | not remeasured in this pass |
| `-n exa no-match-ascii-46m.txt` guardrail | 9.8 ms / 8.5-11.0 ms | 9.8 ms / 8.9-10.4 ms | not remeasured in this pass |
| `-n missingliteral match-ascii-46m.txt` guardrail | 72.0 ms / 69.9-74.8 ms | 71.1 ms / 69.5-72.7 ms | not remeasured in this pass |
| `-q PM_RESUME linux` guardrail | 34.5 ms / 32.2-37.2 ms | 33.6 ms / 31.6-35.2 ms | not remeasured in this pass |

Raw hyperfine exports:
`/tmp/swift-rg-bench/sevenbyte-specialized-probe-1780356700.json` and
`/tmp/swift-rg-bench/sevenbyte-specialized-guard-1780356900.json`.

## Six-byte Swift memmem fixed-shape SIMD helper — 2026-06-01

The no-C-shim Swift memmem fallback now routes six-byte exact literals through
a fixed-shape SIMD16 helper. This covers dense near-misses such as `tqetq `
over the repeated `theta ` fixture, where the generic first/middle/tail
candidate filter accepts every line and then rejects inside the verifier.

Validation:

- Added a six-byte `tqetq ` regression test with repeated `theta` false
  candidates and a positive hit line.
- Current Swift stdout/stderr/status matched checkpoint
  `/tmp/swift-rg-bench/baseline-b8ab69a-1780355893-ripgrep` and Rust for
  six-byte, five-byte, four-byte, three-byte, two-byte, longer-literal, and
  quiet controls. The stats case matched Rust after normalizing elapsed-time
  lines.
- `swift build -c release` and the focused 2/3/4/5/6-byte scanner tests passed
  before benchmarking.

A same-session 30-run hyperfine A/B against checkpoint `b8ab69a`, with five
warmups, measured:

| Command | Current Swift | Checkpoint `b8ab69a` | Rust |
| --- | ---: | ---: | ---: |
| `-n "tqetq " no-match-ascii-46m.txt` | 11.1 ms mean / 10.4-11.9 ms range | 20.7 ms / 19.6-21.4 ms | 8.8 ms / 8.0-9.3 ms |
| `--stats --files-without-match "tqetq " no-match-ascii-46m.txt` | 9.4 ms / 8.4-10.2 ms | 19.2 ms / 17.8-20.9 ms | 7.8 ms / 7.1-8.4 ms |
| `-n zzzzzz no-match-ascii-46m.txt` guardrail | 11.5 ms / 10.7-12.9 ms | 11.4 ms / 10.8-12.7 ms | 8.9 ms / 7.9-9.7 ms |
| `-n tqexa no-match-ascii-46m.txt` guardrail | 10.1 ms / 9.3-10.9 ms | 10.2 ms / 9.2-11.0 ms | not remeasured in this pass |
| `-n btta no-match-ascii-46m.txt` guardrail | 9.9 ms / 9.0-10.6 ms | 9.8 ms / 8.8-10.6 ms | not remeasured in this pass |
| `-n exa no-match-ascii-46m.txt` guardrail | 9.6 ms / 8.7-11.1 ms | 9.4 ms / 8.3-10.0 ms | not remeasured in this pass |
| `-n qz no-match-ascii-46m.txt` guardrail | 9.0 ms / 8.1-9.7 ms | 9.3 ms / 8.4-10.1 ms | not remeasured in this pass |
| `-n missingliteral match-ascii-46m.txt` guardrail | 72.0 ms / 69.9-74.8 ms | 72.1 ms / 70.3-73.5 ms | not remeasured in this pass |
| `-q PM_RESUME linux` guardrail | 33.6 ms / 31.3-35.9 ms | 36.7 ms / 32.5-53.2 ms | not remeasured in this pass |

Raw hyperfine exports:
`/tmp/swift-rg-bench/sixbyte-specialized-probe-1780356100.json` and
`/tmp/swift-rg-bench/sixbyte-specialized-guard-1780356300.json`.

## Five-byte Swift memmem fixed-shape SIMD helper — 2026-06-01

The no-C-shim Swift memmem fallback now routes five-byte exact literals through
a fixed-shape SIMD16 helper as well. This removes generic verifier work for
dense five-byte near-misses such as `tqexa` over the repeated `theta` fixture,
where first, middle, and final bytes match but the second/fourth bytes reject
the candidate.

Validation:

- Added a five-byte `tqexa` regression test where repeated `theta` lines are
  rejected before a real hit file proves the positive path.
- Current Swift stdout/stderr/status matched checkpoint
  `/tmp/swift-rg-bench/baseline-9b73bf2-1780355312-ripgrep` and Rust for
  five-byte, four-byte, three-byte, two-byte, longer-literal, and quiet
  controls. The stats case matched Rust after normalizing the elapsed-time
  lines.
- `swift build -c release` and the focused 2/3/4/5-byte scanner tests passed
  before benchmarking.

A same-session 30-run hyperfine A/B against checkpoint `9b73bf2`, with five
warmups, measured:

| Command | Current Swift | Checkpoint `9b73bf2` | Rust |
| --- | ---: | ---: | ---: |
| `-n tqexa no-match-ascii-46m.txt` | 11.3 ms mean / 10.0-12.0 ms range | 22.0 ms / 21.3-22.9 ms | 9.3 ms / 8.7-10.2 ms |
| `--stats --files-without-match tqexa no-match-ascii-46m.txt` | 9.9 ms / 9.3-10.6 ms | 22.1 ms / 20.9-24.6 ms | 8.8 ms / 8.1-9.4 ms |
| `-n zzzzz no-match-ascii-46m.txt` guardrail | 11.5 ms / 10.7-12.2 ms | 11.9 ms / 11.2-12.5 ms | 9.5 ms / 8.8-10.2 ms |
| `-n btta no-match-ascii-46m.txt` guardrail | 10.7 ms / 9.7-11.7 ms | 10.3 ms / 9.4-11.6 ms | not remeasured in this pass |
| `-n exa no-match-ascii-46m.txt` guardrail | 10.1 ms / 9.4-11.1 ms | 10.0 ms / 9.2-11.7 ms | not remeasured in this pass |
| `-n qz no-match-ascii-46m.txt` guardrail | 9.7 ms / 8.6-11.4 ms | 9.7 ms / 8.8-10.5 ms | not remeasured in this pass |
| `-n missingliteral match-ascii-46m.txt` guardrail | 72.5 ms / 71.1-74.1 ms | 72.0 ms / 70.6-73.5 ms | not remeasured in this pass |
| `-q PM_RESUME linux` guardrail | 35.7 ms / 34.0-38.8 ms | 34.9 ms / 33.6-36.5 ms | not remeasured in this pass |

Raw hyperfine exports:
`/tmp/swift-rg-bench/fivebyte-specialized-probe-1780355500.json` and
`/tmp/swift-rg-bench/fivebyte-specialized-guard-1780355700.json`.
...
## Active literal pruning for multi-literal context — 2026-06-01

The exact case-sensitive multi-literal context proof now records which
repeated-`-e` literals are present while finding the earliest match. After the
binary/NUL text proof succeeds, line scanning uses only those present literals,
so mixed hit/miss context output does not continue testing absent literals on
every line. Binary files that contain any requested literal still decline the
fast path before emitting output.

Validation:

- Current Swift stdout/stderr/status matched checkpoint `183362e` and Rust for
  repeated `-e` `-A`, `-B`, `-C`, line-numbered context, alternation context,
  no-match, binary no-match, and binary literal-match guards.
- `xcrun swift test` passed, including the repeated-`-e` context-output feature
  test, and `SWIFT_RIPGREP_PARITY=1 xcrun swift test --filter ParityHarnessTests` passed.
...
A same-session 20-run hyperfine A/B against checkpoint `183362e`, with three
warmups, measured:

| Command | Current Swift | Checkpoint `183362e` | Rust |
| --- | ---: | ---: | ---: |
| `-A1 -e exa -e xyz no-match-ascii-46m.txt` guardrail | 13.8 ms mean / 13.1-14.4 ms range | 13.6 ms / 13.0-14.7 ms | 10.8 ms / 9.9-12.2 ms |
| `-C1 -e exa -e xyz no-match-ascii-46m.txt` guardrail | 14.2 ms / 12.9-15.4 ms | 13.5 ms / 12.5-14.4 ms | 10.5 ms / 9.6-11.8 ms |
| `-A1 -e literal -e absent match-ascii-46m.txt` | 37.4 ms / 35.6-39.2 ms | 82.1 ms / 79.4-84.1 ms | 50.8 ms / 49.1-52.2 ms |
| `-C1 -e literal -e absent match-ascii-46m.txt` | 42.8 ms / 41.3-44.3 ms | 88.8 ms / 86.4-91.2 ms | 51.2 ms / 49.7-53.4 ms |
| `-C1 -e literal -e alpha match-ascii-46m.txt` both-present guardrail | 32.6 ms / 31.8-33.6 ms | 32.5 ms / 31.6-34.6 ms | not remeasured in this pass |

Raw hyperfine export:
`/tmp/swift-rg-bench/multiliteral-context-active-literals-1780341057.json`.
...
`[A-Z]+<literal>`, `[A-Z]{N}<literal>`, and uppercase singleton repeats like
`[Z]{N}<literal>` now use a Swift byte counter that scans suffix occurrences
and verifies the preceding run without decoding every candidate line. The path
is deliberately stats-only and declines byte-semantics/no-unicode, UTF-16 BOM,
max-count, JSON, context, replacement, and formatting modes back to the
existing matcher. UTF-8 BOM input is scanned after the marker so byte accounting
matches Rust.

Validation:

- Current Swift stdout/stderr/status matched checkpoint `52ed36b` and Rust for
  recursive Linux `--stats -q '[A-Z]+_RESUME'` plus the `[Z]{3}_RESUME` miss
  after normalizing stats elapsed-time lines.
- Current Swift matched Rust for focused `[Z]{3}<literal>` and
  `[A-Z]{3}<literal>` fixtures covering overlapping-looking prefixes,
  non-ASCII text elsewhere on a candidate line, and UTF-8 BOM byte accounting.
- Added Swift regression tests for recursive quiet-stats counts, fixed-repeat
  positives, non-ASCII-containing files, UTF-8 BOM files, and no-match
  summaries.
- The default build remains Swift-only; this change adds no C shim or custom C
  code.

The previous checkpoint had already moved `[A-Z]+_RESUME` from 18.831 s to
roughly 1.67 s. A 2-run hyperfine A/B against checkpoint `52ed36b`, with
1 warmup, measured:

| Command | Current Swift | Checkpoint `52ed36b` | Rust |
| --- | ---: | ---: | ---: |
| `--stats -q '[A-Z]+_RESUME' linux` guardrail | 1.669 s mean / 1.669 s median | 1.667 s / 1.667 s | 2.836 s / 2.836 s |
| `--stats -q '[Z]{3}_RESUME' linux` | 1.703 s / 1.703 s | 18.331 s / 18.331 s | 2.830 s / 2.830 s |

Raw hyperfine exports:
`/tmp/swift-rg-bench/plus-guard-hyperfine-1780314351.json` and
`/tmp/swift-rg-bench/fixed-miss-hyperfine-1780314277.json`.
...
first byte class before entering the full raw scanner. If a file contains no
byte in that class, the search can return the same no-match result without
walking line and count state byte-by-byte. Files with any first-class candidate
still fall through to the existing scanner.

Validation:

- Current Swift stdout/stderr/status matched checkpoint `c65d9a2` and Rust for
  focused `--no-mmap` ASCII fixed-class cases covering recursive line output,
  count output, count-matches output, files-with-matches, files-without-match,
  quiet match/no-match exits, and a 46 MiB no-match file.
- Added a regression assertion for a `--no-mmap` no-candidate fixed-class
  no-match search.
- The default build remains Swift-only; this change uses Swift SIMD and does
  not add or enable any C shim.

A 12-run hyperfine A/B against checkpoint `c65d9a2`, with 3 warmups, measured:

| Command | Current Swift | Previous Swift | Rust |
| --- | ---: | ---: | ---: |
| `--no-mmap -n '[A-Z]{5}' no-match-ascii-46m.txt` | 56.67 ms mean / 55.27 ms median | 81.65 ms / 80.98 ms | 19.78 ms / 19.90 ms |
| `-n '[A-Z]{5}' no-match-ascii-46m.txt` guardrail | 52.69 ms / 54.51 ms | 83.47 ms / 82.52 ms | not remeasured in this pass |
| `--no-mmap -c '[A-Z]{5}' no-match-ascii-46m.txt` | 40.31 ms / 41.38 ms | 73.05 ms / 73.50 ms | not remeasured in this pass |

Raw hyperfine export:
`/tmp/swift-rg-bench/ascii-fixed-first-class-proof-1780300442.json`.
